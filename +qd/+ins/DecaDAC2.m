...
% know better, call obj.set_all_to_4channel_mode in your setup script.
    properties
        ...
        % Offset of all channels. Format:
        % offset.CH0 = 0. Default is 0
        offset = struct

        % Divider factor of all channels.
        % The divider factor is to compensate for voltage dividers, etc.
        % Format:
        % divider.CH0 = 1. Default is 1
        divider = struct
    ...
    methods
        ...
        function future = setc_async(obj, ch, value)
            value = value*obj.divider.(ch) - obj.offset.(ch);               % Modify value based on offset and divider
            
            n = obj.parse_ch(ch);
            if ~obj.mode_set(floor(n/4) + 1)
                obj.emit_mode_warning(ch);
            end
            obj.validate(ch, value);
            ...
            if ~isempty(obj.ramp_rates.(ch))
                ...
                future = obj.set_no_ramp(ch, n, value);
            end
        end

        function setDivider(obj, channel, divider)
            if ~any(strcmpi(channel,obj.channels))                          % Checking that channel exists
                error('Not supported.')
            else
        ...
        function setOffset(obj, channel, offset)
            if ~any(strcmpi(channel,obj.channels))                          % Checking that channel exists
                ...
                obj.offset.(channel) = offset;                              % Set divider
            end
        end
        
        % Function to calibrate the offset and division
        function calibrateChannel(obj,channel,data)
            
            % We only support 1D data, and only one channel
            if length(data) > 2
                error('DAC calibration supports just data of 1D single, channel measurements')
            end

            % Perform fitting
            fitObj = fit(data{1}.data,data{2}.data,'poly1');
