classdef NIdaq < qd.classes.ComInstrument
% Drivers for our NIdaqs.
    properties
    ...
    methods
        function obj = NIdaq(devNumber)
            devices = daq.getDevices;
            obj.dev = devices(devNumber);

            obj.s = daq.createSession('ni');

            nOfSubsystems = length(obj.dev.Subsystems);
            for i=1:nOfSubsystems
                if strcmp(obj.dev.Subsystems(i).SubsystemType, 'AnalogOutput')
                    obj.subsystemNumber = i;
                    break
                end
            end

            subsystem = obj.dev.Subsystems(obj.subsystemNumber);
            channelNames = subsystem.ChannelNames;
            daqName = obj.dev.ID;
            for ch = obj.channels
                n = obj.parse_ch(ch{1});
                addAnalogOutputChannel(obj.s, daqName, n, 'Voltage');
        ...
        function zeroAll(obj)
            obj.outputValues = zeros(1,obj.number_of_channels);
            obj.setc('CH0',0);
        end
        ...
        function future = setc_async(obj, ch, val)
            ...
            % Ramps are not yet implemented for the NIdaq.
    ...
    methods(Access=private)
        ...
        % n is parse_ch(ch).
            obj.outputValues(n+1) = val;
            outputSingleScan(obj.s, obj.outputValues);
